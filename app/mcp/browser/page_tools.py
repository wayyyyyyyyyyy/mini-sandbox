...
class BrowserPageMcpTools:
        ...
        return {
            "browser_navigate": McpTool(
                ...
                input_schema={
                    ...
                    "properties": {
                        "url": {"type": "string"},
                        "wait_until": {"type": "string", "enum": sorted(_WAIT_UNTIL_VALUES)},
    ...
    def browser_navigate(self, arguments: dict[str, Any]) -> McpCallToolResult:
        ...
        timeout = optional_int_range(arguments, "timeout", default=30000, minimum=1000, maximum=120000)
        return json_result(self.browser_sessions.navigate(url=url, wait_until=wait_until, timeout=timeout))

    def browser_text(self, _: dict[str, Any]) -> McpCallToolResult:
        return McpCallToolResult(content=[McpContentItem(type="text", text=self.browser_sessions.text())])
